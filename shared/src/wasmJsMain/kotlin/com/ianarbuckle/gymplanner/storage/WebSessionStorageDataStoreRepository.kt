...
class WebSessionStorageDataStoreRepository : DataStoreRepository {
    ...
    override suspend fun getStringData(key: Preferences.Key<String>): String? = sessionStorage.getItem(key.name)

    override suspend fun getBooleanData(key: Preferences.Key<Boolean>): Boolean? = sessionStorage.getItem(key.name)?.toBoolean()
