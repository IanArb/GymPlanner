...
data class AvailabilityDto(
    ...
    fun toAvailability(): Availability = Availability(
        id = id,
        month = month,
    ...
    private fun SlotDto.toSlot(): Slot = Slot(date = date, id = id, times = times.map { it.toTime() }.toImmutableList())

    private fun TimeDto.toTime(): Time = Time(id = id, startTime = startTime, endTime = endTime, status = status)
}

@Serializable data class SlotDto(
    val id: String,
    val date: String,
