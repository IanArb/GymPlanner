...
object AuthenticationTestDataProvider {
    ...
    object LoginResponses {
        val default = LoginResponseDto(
            userId = "test-user-id",
            token = "test-token",
            expiration = 1234567890L
        )

        val valid = LoginResponseDto(
            userId = "user123",
            token = "jwt-token-abc",
            expiration = 1735689600000L
        )

        val mapped = LoginResponseDto(
            userId = "mapped-user",
            token = "mapped-token",
            expiration = 9999999999L
        )

        val user2Response = LoginResponseDto(
            userId = "user2-id",
            token = "user2-token",
            expiration = 8888888888L
        )

        val zeroExpiration = LoginResponseDto(
            userId = "id",
            token = "token",
            expiration = 0L
        )

        val negativeExpiration = LoginResponseDto(
            userId = "id",
            token = "token",
            expiration = -1L
        )
    ...
    object RegisterResponses {
        ...
        val secondRegistration = RegisterResponseDto(
            message = "Second registration successful"
        )
    }
