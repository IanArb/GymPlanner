...
class AvailabilityViewModelTests {
  ...
  fun `init should update state to AvailabilitySuccess when API calls succeed`() = runTest {
    ...
    coEvery {
      availabilityRepository.checkAvailability(personalTrainerId, currentDateTime.calendarMonth())
    } returns
      Result.success(CheckAvailability(personalTrainerId = personalTrainerId, isAvailable = true))
    coEvery {
      availabilityRepository.getAvailability(personalTrainerId, currentDateTime.calendarMonth())
