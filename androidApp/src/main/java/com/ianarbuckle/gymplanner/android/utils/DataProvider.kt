...
object DataProvider {
    fun client(
        ...
        gymPlan =
        gymPlan(name = name, startDate = startDate, endDate = endDate, sessions = sessions),
    )
    ...
    fun fitnessClasses(): ImmutableList<FitnessClass> = persistentListOf(
        FitnessClass(
            dayOfWeek = "MONDAY",
            name = "Pilates Class",
    ...
    fun gymLocations(): ImmutableList<GymLocations> = persistentListOf(
        GymLocations(
            title = "Clontarf",
    ...
    ): CheckAvailability = CheckAvailability(personalTrainerId = personalTrainerId, isAvailable = isAvailable)
    ...
    fun bookings(): ImmutableList<BookingResponse> = persistentListOf(bookingResponse())
}
